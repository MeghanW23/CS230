...
@Path("/gameusers") // add @path annotation to show to path for /gameusers
...
public class GameUserRESTController {
    ...
    @RolesAllowed({"USER"}) // allow users with the USER role to access this method
    @GET
    @Path("/{id}")
    ...
    @RolesAllowed ({"ADMIN"}) // allow ADMIN role only to access this method
    @POST
