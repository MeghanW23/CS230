...
public class GameAuthApplication extends Application<Configuration> {
	...
	public void run(Configuration c, Environment e) throws Exception 
	{
		...
		e.jersey().register(new GameUserRESTController(e.getValidator())); // register a GameUserRESTController instance and pass a validator from the Dropwizard environment as a param
		...
		final Client client = new JerseyClientBuilder(e)
				.build("DemoRESTClient"); // create http client named DemoRESTClient, passing the dopwizard environment 
		e.jersey().register(new RESTClientController(client)); // register the client as a dropwizard resource
